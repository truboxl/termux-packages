...
termux_step_make() {
	...
	#./eng/build.sh --cross --arch "${arch}" --subset clr+libs+host+packs --os linux-bionic
	./build.sh --arch "${arch}" --subset clr+libs+host+packs --os linux-bionic --ninja
	git status
	popd
	...
	rm -fr .dotnet
	ln -fsv ../../.dotnet .dotnet
	./build.sh --pack /p:Projects=${PWD}/source-build.slnf /p:Architecture=${arch} /p:TargetOS=linux-bionic
	git status
	popd
	...
	rm -fr .dotnet
	ln -fsv ../../.dotnet .dotnet
	warn_as_error=false DotNetBuildFromSource=true ./eng/build.sh --pack --arch "${arch}" --no-build-nodejs /p:TargetOS=linux-bionic /warnnotaserror /p:TreatWarningsAsErrors=false
	popd

	pushd src/installer
	rm -fr .dotnet
	ln -fsv ../../.dotnet .dotnet
	./build.sh --arch "${arch}" --os linux-bionic
	popd
}
...
termux_step_post_make_install() {
	...
	for component in runtime sdk aspnetcore installer; do
		echo "INFO: ${component}"
		[[ ! -d "${TERMUX_PKG_BUILDDIR}/src/${component}/artifacts" ]] && "INFO: No artifacts folder" && continue

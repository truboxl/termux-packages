...
termux_step_make() {
	# TODO work on full source build that may (or not) fix below:
	# PublishAot true is broken
	# PublishSingleFile true is broken
	# PublishSelfContained true needs to set LD_LIBRARY_PATH to link libc++_shared.so, see TODO
	# dotnet build *.sln is broken / stuck for a long time with no real error, dotnet build *.csproj instead
...
termux_step_make_install() {
	...
	if :; then
	cp -fr "${_downloaddir}" "${TERMUX_PREFIX}/opt/${TERMUX_PKG_NAME}"
	du -sh "${TERMUX_PREFIX}/opt/${TERMUX_PKG_NAME}"
	fi
	fi

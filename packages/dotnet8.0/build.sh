...
termux_step_make() {
	...
	# TODO aspnetcore and installer builds using linux- instead of linux-bionic-
	#return
...
termux_step_make_install() {
	...
	# TODO likely downloaded from Microsoft and not built
	local tgz
	...
	mkdir -p coreclr
	cp -frv ${TERMUX_PKG_BUILDDIR}/src/runtime/artifacts/bin/coreclr/linux.arm64.Release/* coreclr
	# copy nupkg files
	...
	# this is needed to link libc++_shared.so, libssl.so.3, etc
	...
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name 'apphost' -exec chmod +x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name 'singlefilehost' -exec chmod +x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name 'lib*so' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.a' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.dll' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.h' -exec chmod 0644 {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.json' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.pdb' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.props' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.pubxml' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.targets' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.txt' -exec chmod -x {} \;
	find "${TERMUX_PREFIX}"/lib/dotnet/ -type f -name '*.xml' -exec chmod -x {} \;
}

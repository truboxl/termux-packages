/*
 *   Stub libopencl that dlsyms into actual library based on environment
 *   variable
 *
...
#if !defined(DEFAULT_LIBOPENCL_SO)
#define DEFAULT_LIBOPENCL_SO "libOpenCL.so"
#endif

static const char *default_so_paths[] = {DEFAULT_LIBOPENCL_SO};
...
cl_int clCreateSubDevices(cl_device_id in_device, const cl_device_partition_property *properties,
...
cl_int clGetContextInfo(cl_context context, cl_context_info param_name, size_t param_value_size,
                        void *param_value, size_t *param_value_size_ret) {
...
cl_command_queue clCreateCommandQueueWithProperties(cl_context context, cl_device_id device,
                                                    ...
                                                    cl_int *errcode_ret) {
    f_clCreateCommandQueueWithProperties func = NULL;
    ...
    if (so_handle) {
        func = (f_clCreateCommandQueueWithProperties)dlsym(so_handle,
...
cl_int clGetSupportedImageFormats(cl_context context, cl_mem_flags flags,
    ...
    if (func) {
        return func(context, flags, image_type, num_entries, image_formats, num_image_formats);
    } else {
        return CL_INVALID_PLATFORM;
    }
}
...
cl_int clGetImageInfo(cl_mem image, cl_image_info param_name, size_t param_value_size,
                      void *param_value, size_t *param_value_size_ret) {
...
cl_int clSetMemObjectDestructorCallback(cl_mem memobj,
    ...
    if (so_handle) {
        func = (f_clSetMemObjectDestructorCallback)dlsym(so_handle,
...
cl_program clCreateProgramWithBinary(cl_context context, cl_uint num_devices,
    ...
    if (func) {
        return func(context, num_devices, device_list, lengths, binaries, binary_status,
                    errcode_ret);
    } else {
        return NULL;
    }
...
cl_program clCreateProgramWithBuiltInKernels(cl_context context, cl_uint num_devices,
    ...
    if (so_handle) {
        func = (f_clCreateProgramWithBuiltInKernels)dlsym(so_handle,
...
cl_int clCompileProgram(cl_program program, cl_uint num_devices, const cl_device_id *device_list,
...
cl_program clLinkProgram(cl_context context, cl_uint num_devices, const cl_device_id *device_list,
    ...
    if (func) {
        return func(context, num_devices, device_list, options, num_input_programs, input_programs,
...
cl_int clGetKernelInfo(cl_kernel kernel, cl_kernel_info param_name, size_t param_value_size,
                       void *param_value, size_t *param_value_size_ret) {
...
cl_int clGetKernelArgInfo(cl_kernel kernel, cl_uint arg_indx, cl_kernel_arg_info param_name,
                          size_t param_value_size, void *param_value,
                          size_t *param_value_size_ret) {
...
cl_int clEnqueueFillImage(cl_command_queue command_queue, cl_mem image, const void *fill_color,
    ...
    if (func) {
        return func(command_queue, image, fill_color, origin, region, num_events_in_wait_list,
...
void *clEnqueueMapImage(cl_command_queue command_queue, cl_mem image, cl_bool blocking_map,
    ...
    if (func) {
        return func(command_queue, image, blocking_map, map_flags, origin, region, image_row_pitch,
                    image_slice_pitch, num_events_in_wait_list, event_wait_list, event,
                    errcode_ret);
    } else {
        return NULL;
    }
...
cl_int clEnqueueUnmapMemObject(cl_command_queue command_queue, cl_mem memobj, void *mapped_ptr,
    ...
    if (func) {
        return func(command_queue, memobj, mapped_ptr, num_events_in_wait_list, event_wait_list,
                    event);
    } else {
        return CL_INVALID_PLATFORM;
    }
...
cl_int clEnqueueMigrateMemObjects(cl_command_queue command_queue, cl_uint num_mem_objects,
    ...
    if (func) {
        return func(command_queue, num_mem_objects, mem_objects, flags, num_events_in_wait_list,
                    event_wait_list, event);
    } else {
        return CL_INVALID_PLATFORM;
    }
...
cl_int clEnqueueNativeKernel(cl_command_queue command_queue, void (*user_func)(void *), void *args,
...
cl_mem clCreateImage2D(cl_context context, cl_mem_flags flags, const cl_image_format *image_format,
    ...
    if (func) {
        return func(context, flags, image_format, image_width, image_height, image_row_pitch,
...
cl_mem clCreateImage3D(cl_context context, cl_mem_flags flags, const cl_image_format *image_format,
...
cl_int clEnqueueWaitForEvents(cl_command_queue command_queue, cl_uint num_events,
...
cl_mem clCreateFromGLBuffer(cl_context context, cl_mem_flags flags, cl_GLuint bufobj,
                            int *errcode_ret) {
...
cl_mem clCreateFromGLTexture(cl_context context, cl_mem_flags flags, cl_GLenum target,
...
cl_mem clCreateFromGLRenderbuffer(cl_context context, cl_mem_flags flags, cl_GLuint renderbuffer,
                                  cl_int *errcode_ret) {
...
cl_int clGetGLTextureInfo(cl_mem memobj, cl_gl_texture_info param_name, size_t param_value_size,
                          void *param_value, size_t *param_value_size_ret) {
...
cl_int clEnqueueAcquireGLObjects(cl_command_queue command_queue, cl_uint num_objects,
    ...
    if (func) {
        return func(command_queue, num_objects, mem_objects, num_events_in_wait_list,
                    event_wait_list, event);
    } else {
        return CL_INVALID_PLATFORM;
    }
...
cl_mem clCreateFromGLTexture2D(cl_context context, cl_mem_flags flags, cl_GLenum target,
                               cl_GLint miplevel, cl_GLuint texture, cl_int *errcode_ret) {
...
cl_mem clCreateFromGLTexture3D(cl_context context, cl_mem_flags flags, cl_GLenum target,
                               cl_GLint miplevel, cl_GLuint texture, cl_int *errcode_ret) {
...
cl_int clIcdGetPlatformIDsKHR(cl_uint num_entries, cl_platform_id *platforms, cl_uint *num_platforms) {
    f_clIcdGetPlatformIDsKHR func = NULL;
    ...
    if (so_handle) {
        func = (f_clIcdGetPlatformIDsKHR)dlsym(so_handle, "clIcdGetPlatformIDsKHR");
    }

    if (func) {
        return func(num_entries, platforms, num_platforms);
    } else {
        return func(1, platforms, num_platforms);
    }
}

termux_step_massage() {
	...
	if [ "$TERMUX_PACKAGE_LIBRARY" = "bionic" ]; then
		if [ "$TERMUX_PKG_NO_STRIP" != "true" ] && [ "$TERMUX_DEBUG_BUILD" = "false" ]; then
			...
			set +e +o pipefail
			find . \( -path "./bin/*" -o -path "./lib/*" -o -path "./libexec/*" \) -type f -print0 |
				xargs -0 -r file | grep -E "ELF .+ (executable|shared object)" | cut -f 1 -d : |
		...
		if [ "$TERMUX_PKG_NO_ELF_CLEANER" != "true" ]; then
			...
			find . \( -path "./bin/*" -o -path "./lib/*" -o -path "./libexec/*" -o -path "./opt/*" \) -type f -print0 | xargs -0 -r \
	...
	if [ -d ./${ADDING_PREFIX}share/man ]; then
		...
		find ./${ADDING_PREFIX}share/man -mindepth 1 -maxdepth 1 -type d ! -name man\* -print0 | xargs -0 -r rm -rf

		# Compress man pages with gzip:
		find ./${ADDING_PREFIX}share/man -type f ! -iname \*.gz -print0 | xargs -0 -r gzip -9 -n
	...
	if [[ "${TERMUX_PACKAGE_LIBRARY}" == "bionic" ]]; then
		...
		local valid=$(find . -type f -print0 | xargs -0 -r -P"${nproc}" -i bash -c 'if ${READELF} -h "{}" &>/dev/null; then echo "{}"; fi')
		local t1=$(get_epoch)
		echo "INFO: Done ... $((t1-t0))s"
		...
		if [[ "${numberOfValid}" != 0 ]]; then
			...
			local t0=$(get_epoch)
			local undef=$(echo "${valid}" | xargs -r -P"${nproc}" -i sh -c '${READELF} -s "{}" | grep -Ef "${pattern_file_undef}"')
			local openmp=$(echo "${valid}" | xargs -r -P"${nproc}" -i sh -c '${READELF} -s "{}" | grep -Ef "${pattern_file_openmp}"')
			local depend_libomp_so=$(echo "${valid}" | xargs -r -P"${nproc}" -n1 ${READELF} -d 2>/dev/null | sed -ne "s|.*NEEDED.*\[\(.*\)\].*|\1|p" | grep libomp.so)
			local t1=$(get_epoch)
			echo "INFO: Done ... $((t1-t0))s"
		fi
		...
		local subpackage_platform_independent_explicit=$(echo "${subpackage_platform_independent}" | cut -d"=" -f2 | sort | uniq | tail -n1) # check for true
		local error=0
		...
		true:true::*) ;; # override
		true:true:false:0) ;;
		true:true:false:*) ;; # override (ndk-multilib)
		true:true:true:0) ;; # override
		true:true:true:*) ;; # override (emscripten)
		*)

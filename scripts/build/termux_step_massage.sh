termux_step_massage() {
	...
	# installed there.
	if [ "$(find . -path "./${ADDING_PREFIX}share/doc/$TERMUX_PKG_NAME" -prune -o -type f -print 2>/dev/null | head -n1)" = "" ]; then
	...
	if [[ "${TERMUX_PACKAGE_LIBRARY}" == "bionic" ]]; then
		...
		echo "INFO: Done under $((t1-t0))s"
		echo "INFO: Total symbols $(echo "${SYMBOLS}" | wc -w)"
		local t0=$(cut -d"." -f1 /proc/uptime)
		echo "INFO: Running symbol checks"
		local c=0
		local e=0
		local libs=$(find . ! -type d -name "*.so*")
		for lib in ${libs}; do
			if ! "${READELF}" -h "${lib}" &>/dev/null; then continue; fi
			if "${READELF}" -sW "${lib}" | grep -E "${grep_pattern}" &>/dev/null; then
				echo "ERROR: ${lib} contains undefined symbols:\n$(${READELF} -sW "${lib}" | grep -E "${grep_pattern}")" >&2
				e=1
			fi
			c=$((c+1))
		done
		if [[ "${e}" != 0 ]]; then termux_error_exit "Refer above"; fi
		local t1=$(cut -d"." -f1 /proc/uptime)
		echo "INFO: Done under $((t1-t0))s"
		echo "INFO: Valid libs ${c} of total files $(echo "${libs}" | wc -l)"
	fi

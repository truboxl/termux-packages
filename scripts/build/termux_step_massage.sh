termux_step_massage() {
	...
	# installed there.
	local files=$(find . -path "./${ADDING_PREFIX}share/doc/${TERMUX_PKG_NAME}" -prune -o -type f -print)
	if [[ "$(echo "${files}" | head -n1)" = "" ]]; then
	...
	if [[ "${TERMUX_PACKAGE_LIBRARY}" == "bionic" ]]; then
		...
		for lib in ${libs}; do
			...
			if "${READELF}" -sW "${lib}" | grep -E "${grep_pattern}" &>/dev/null; then
				echo -e "ERROR: ${lib} contains undefined symbols:\n$(${READELF} -sW "${lib}" | grep -E "${grep_pattern}")" >&2
				e=1
			fi
			c=$((c+1))
		done
		local t1=$(cut -d"." -f1 /proc/uptime)
		echo "INFO: Done ... $((t1-t0))s"
		echo "INFO: Checked ${c} / $(echo "${libs}" | wc -l) files"
		echo -e "INFO: Excluded files:\n$(echo ${excluded} | tr " " "\n")"

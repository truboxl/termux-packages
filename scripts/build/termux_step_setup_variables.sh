termux_step_setup_variables() {
	...
	TERMUX_PKG_BUILD32=false # cross compilation (compilation of 32-bit packages for 64-bit devices)
	TERMUX_PKG_ONLY_BUILD32=false # Specifies that the package is compiled only via cross compilation. Enabled automatically if cross compilation is enabled and the `TERMUX_PKG_EXCLUDED_ARCHES` variable contains `arm` and `i686` values.
	TERMUX_PKG_BUILD32DIR=$TERMUX_TOPDIR/$TERMUX_PKG_NAME/build32 # path to the assembled components of the 32-bit package if cross-compilation is enabled
